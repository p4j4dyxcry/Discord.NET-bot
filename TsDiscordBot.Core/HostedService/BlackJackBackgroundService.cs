...
public class BlackJackBackgroundService(
    DiscordSocketClient client,
    ILogger<BlackJackBackgroundService> logger,
    DatabaseService databaseService) : BackgroundService
{
    private readonly DiscordSocketClient _client = client;
    private readonly ILogger<BlackJackBackgroundService> _logger = logger;
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                var plays = _databaseService
                    ...
                    .Where(x => x.GameKind == "BJ" && !x.Started)
                    .ToArray();
